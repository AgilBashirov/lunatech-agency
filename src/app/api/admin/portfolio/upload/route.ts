/**
 * Admin Route Handler — portfolio image upload.
 *
 * POST /api/admin/portfolio/upload
 *   Accepts multipart/form-data with a `file` field.
 *   Uploads the file to Vercel Blob under `portfolio-images/{id}-{timestamp}.ext`.
...
// ---------------------------------------------------------------------------
// Allowed MIME types for portfolio cover images
// ---------------------------------------------------------------------------
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (fileEntry.size > MAX_SIZE_BYTES) {
    return Response.json(
      {
        ok: false,
        error: `Fayl ölçüsü limitdən (5 MB) böyükdür.`,
      },
      { status: 413 },
    );
  }
  ...
  const path = `portfolio-images/${id}-${Date.now()}.${ext}`;
